...
def load_to_big_query_cross_border_flow(date, country_code_from, country_code_to, table):
    ...
    uri = f"gs://energy-grid-pipeline-raw/cross_border_flow/from_{country_code_from}_to_{country_code_to}/{day_str}/data.parquet"
...
def fetch_crossborder_flow(country_code_from, country_code_to, start, end):
    token = os.getenv("ENTSOE_TOKEN")
    ...
    for day, group in days:
        if group.empty:
            continue
        day_str = day.strftime('%Y-%m-%d')
        gcs_path = f"gs://energy-grid-pipeline-raw/cross_border_flow/from_{country_code_from}_to_{country_code_to}/{day_str}/data.parquet"
        group.to_parquet(gcs_path)
